...
def plot_solution(sol):
    ...
    ax.set_title(f"{wvlen} nm Collimated Mode Profile")
    ...
    # Label the lens focal lengths at the very top of the plot
    y_top = ymax * 1.05
    ax.text(lens1_x, y_top, f"f1={f1*1e3:.3f} mm", ha="center", va="bottom", fontsize=10, fontweight="bold", color="C1")
    ax.text(lens2_x, y_top, f"f2={f2*1e3:.3f} mm", ha="center", va="bottom", fontsize=10, fontweight="bold", color="C1")

    # Add distance labels below the x-axis for clarity
    ax.text(lens1_x, -0.07, f"d1={d1*1e3:.12g}mm", transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=11, color="black", fontweight="bold", clip_on=False)
    ax.text(lens2_x, -0.07, f"d1+d12={lens2_x:.12g}mm", transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=11, color="black", fontweight="bold", clip_on=False)

    # Place the d2 labels above the plot area
    ax.text(d2_x, 1.02, f"d2={d2*1e3:.2f}mm", transform=ax.get_xaxis_transform(), ha="center", va="bottom", fontsize=11, color="red", fontweight="bold", clip_on=False)
    if ideal_d2_x is not None:
        ax.text(ideal_d2_x, 1.06, f"Waist at = {d2_ideal*1e3:.2f}mm", transform=ax.get_xaxis_transform(), ha="center", va="bottom", fontsize=9, color="tab:green", fontweight="bold", clip_on=False)
    ...
    # The center info box with ROC and error metrics is intentionally disabled.
    ...
    out_name = PLOTS_DIR / f"{wvlen}_nm_collimated_mode_profile.png"
    fig.tight_layout()
